...
claude mcp github create-pr \
  ...
  --body "$(cat <<'EOF'
## Summary
🎯 Complete user authentication endpoint for Smart Budget Intelligence feature
...
- ✅ Security tests: 8 tests passing
...
🤖 Generated by MoneyWise Agent Orchestration System
EOF
)"
```
...
claude mcp github create-issue \
  --title "🚫 Test Failure: $(get-failed-test-name)" \
...
claude mcp github create-hotfix \
  --issue-number "789" \
  --title "🚨 Critical: Fix security vulnerability in JWT validation" \
...
# 3. Test setup
claude mcp test github && echo "✅ GitHub MCP ready"
claude mcp test github-actions && echo "✅ GitHub Actions MCP ready"
claude mcp test github-security && echo "✅ GitHub Security MCP ready"
