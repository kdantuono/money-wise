...
export class AddTimescaleDBSupport1759998888888 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        // This is safe to run even if already a hypertable
        await queryRunner.query(`SELECT create_hypertable('transactions', 'date', if_not_exists => TRUE);`);

        // Create additional indexes optimized for time-series queries
        await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_transactions_time_bucket
            ON transactions (time_bucket('1 day', date), "accountId");`);

        await queryRunner.query(`CREATE INDEX IF NOT EXISTS idx_transactions_time_bucket_category
            ON transactions (time_bucket('1 day', date), "categoryId");`);

        // Add compression policy for transactions older than 7 days
        await queryRunner.query(`SELECT add_compression_policy('transactions', INTERVAL '7 days', if_not_exists => TRUE);`);

        // Add retention policy for transactions older than 7 years (for compliance)
        await queryRunner.query(`SELECT add_retention_policy('transactions', INTERVAL '7 years', if_not_exists => TRUE);`);
        ...
        await queryRunner.query(`SELECT add_continuous_aggregate_policy('daily_account_balances',
            start_offset => INTERVAL '3 days',
            end_offset => INTERVAL '1 hour',
            schedule_interval => INTERVAL '1 hour',
            if_not_exists => TRUE);`);

        await queryRunner.query(`SELECT add_continuous_aggregate_policy('daily_category_spending',
            start_offset => INTERVAL '3 days',
            end_offset => INTERVAL '1 hour',
            schedule_interval => INTERVAL '1 hour',
            if_not_exists => TRUE);`);
