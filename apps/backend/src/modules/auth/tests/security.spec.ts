...
describe('Security Framework Tests', () => {
  ...
  beforeEach(async () => {
    ...
    mfaRepository = module.get<Repository<UserMfaSettings>>(
      getRepositoryToken(UserMfaSettings)
    );
  ...
  describe('MFA Security Tests', () => {
    describe('TOTP Implementation Security', () => {
      it('should generate cryptographically secure TOTP secrets', async () => {
        ...
        jest
          .spyOn(mfaRepository, 'create')
          .mockReturnValue({} as UserMfaSettings);
        jest
          .spyOn(mfaRepository, 'save')
          .mockResolvedValue({} as UserMfaSettings);
  ...
  describe('Session Security Tests', () => {
    describe('JWT Token Security', () => {
      ...
      it('should enforce token expiration', async () => {
        ...
        const tokens = await sessionService.generateTokenPair(
          userId,
          deviceInfo
        );
    ...
    describe('Session Hijacking Protection', () => {
      it('should detect suspicious session activity', async () => {
        ...
        const securityCheck = await sessionService.validateTokenSecurity(
          session,
          suspiciousDevice
        );
  ...
  describe('Security Threat Detection', () => {
    it('should detect brute force attempts', async () => {
      ...
      const threat =
        await securityService.detectSuspiciousActivity(mockRequest);
