...
const CardLogo = ({
  ...
  if (firstFour.startsWith('4')) {
    return (
      <div
        className={`${className} flex items-center justify-center bg-blue-600 text-white text-xs font-bold rounded`}
      >
        VISA
      </div>
    );
  ...
  if (firstFour.startsWith('5')) {
    return (
      <div
        className={`${className} flex items-center justify-center bg-red-600 text-white text-xs font-bold rounded`}
      >
        MC
      </div>
    );
  ...
  if (firstFour.startsWith('3')) {
    return (
      <div
        className={`${className} flex items-center justify-center bg-green-600 text-white text-xs font-bold rounded`}
      >
        AMEX
      </div>
    );
      ...
      >
        ...
        <div className='relative z-10 flex justify-between items-start mb-6'>
          <div className='space-y-1'>
            <div className='flex items-center space-x-2'>
              <p
                className={`text-xs font-medium ${isDark ? 'text-blue-200' : 'text-neutral-500'}`}
              >
                {card.bankName}
              </p>
              {card.isMain && (
            ...
            <div className='space-y-1'>
              <p
                className={`text-xs ${isDark ? 'text-blue-200' : 'text-neutral-500'}`}
              >
                Current Balance
              </p>
              <div className='flex items-center space-x-2'>
        ...
        >
          <div className='flex items-center space-x-3'>
            <div
              className={`w-8 h-6 rounded ${isDark ? 'bg-gradient-to-r from-yellow-400 to-yellow-500' : 'bg-gradient-to-r from-yellow-400 to-yellow-500'}`}
            >
          ...
          <div className='flex space-x-1'>
            <div
              className={`w-6 h-6 rounded-full ${isDark ? 'bg-white/40' : 'bg-neutral-400/40'}`}
            ></div>
            <div
              className={`w-6 h-6 rounded-full -ml-2 ${isDark ? 'bg-white/60' : 'bg-neutral-400/60'}`}
            ></div>
          </div>
        </div>
        ...
        <div className='absolute bottom-2 left-6 right-6 z-10'>
          ...
          <div
            className={`w-full h-1 rounded-full ${isDark ? 'bg-white/20' : 'bg-neutral-200'}`}
          >
            <div
      ...
      >
        ...
        <div className='text-center'>
          <p className='text-lg font-bold text-primary-600 dark:text-primary-400'>
            {cardData
              .reduce((sum, card) => sum + card.rewards, 0)
              .toLocaleString()}
          </p>
          <p className='text-xs text-neutral-500 dark:text-neutral-400'>
            Rewards
